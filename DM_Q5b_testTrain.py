...
def TreeGrowthRecursive(dataset,validationSet,noOfCols):
    if(stoppingCondition(dataset,noOfCols) == True or len(dataset) == 1):
        ...
        val_splits=splitDataset(validationSet,bestAttbr[1],int(root.nodeAttbr))
        label1=classifyLabel(val_splits[0], noOfCols)
        label2=classifyLabel(val_splits[1], noOfCols)
        acc1=calculateAcc(val_splits[0], label1)
        acc2=calculateAcc(val_splits[1], label2)
        if(acc1+acc2/2)>=0.3:
            root.left=TreeGrowthRecursive(splits[0],noOfCols)
            root.right=TreeGrowthRecursive(splits[1],noOfCols)
        else:
...
for i in range(0,10):
    print "Training: ",i
    TrainData, TestData=sklearn.cross_validation.train_test_split(training_test[0][i],test_size=0.25 )
    root=TreeGrowthRecursive(TrainData,num_cols)
    showData(root)
    print "\n"
    predClassCol=predict(root,TestData)
    perf_eval=createConfusionMat(TestData,predClassCol)
