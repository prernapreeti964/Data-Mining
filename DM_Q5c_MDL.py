...
def MDL(numLeaves,numInternal, subset1,subset2,dataset):
    dataset=np.array(dataset)
    uniqueClassValues= np.unique(dataset[:,-1])
    leftTreeCnt=maximumClass(subset1,len(subset1[0]))
    rightTreeCnt=maximumClass(subset2,len(subset2[0]))
    costLeaves=numLeaves*math.ceil(math.log(len(uniqueClassValues),2))
    costInternal=numInternal*math.log(len(dataset[0])-1)
    errCost=(leftTreeCnt+rightTreeCnt)*(math.log(len(dataset),2))
    minimumDescriptionLength=costLeaves+costInternal+errCost
    return minimumDescriptionLength
...
def TreeGrowthRecursive(dataset,noOfCols,errParent):
    ...
    if(stoppingCondition(dataset,noOfCols) == True or len(dataset) == 1):
        ...
        error=MDL(countLeaves, numIntNodes, splits[0],splits[1], dataset)
        if(error<=errParent):
